...
(defmacro ^:private run-benchmarks
  ...
  [measure file]
  `(do
     (annotate ~measure (cheshire/parse-stream (io/reader ~file)))
     ...
     ;; NOTE: pjson is disabled due to https://github.com/gerritjvv/pjson/issues/42
     ;; It can't handle autogenerated JSON with backslashes all over the place
     #_(let [s (slurp ~file)]
         (annotate ~measure-macro (pjson/read-str s)))))
...
(defn- parsers-agree
  ...
  [file]
  (= (cheshire/parse-stream (io/reader file))
